...
def getAuth():
    ...
    accountType = findUserQuery.get("accountType")

    findUserRestaurantQuery = database["restaurants"].find_one({"userID": session.get("userID")}, {"_id": False})

    return jsonify({"status": 200, "userID": userID, "accountType": accountType, "restaurant": findUserRestaurantQuery})
...
def login():
    ...
    if findUserQuery.get("password") == encryptedPassword:
        ...
        database["sessions"].insert_one({"sessionToken": sessionToken, "email": email, "userID": findUserQuery.get("userID")})

        return jsonify({"status": 200, "sessionToken": sessionToken, "userID": findUserQuery.get("userID"), "accountType": findUserQuery.get("accountType")})
